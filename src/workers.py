...
class VtuBer:
    ...
    async def generate_chat(self):
        ...
        llm_res = await chat.agenerate([self.event.prompt_messages])
    ...
    async def generate_action(self):
        if isinstance(self.event.action, str):
            ...
            # 匹配动作
            self.event.action = top_n_indices_from_embeddings(embedding, live2D_embeddings, top=1)[0]
    ...
    async def output(self):
        ...
        # 播放动作
        if isinstance(self.event.action, int):
